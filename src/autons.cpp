...
int screen = 0;
int intake_score_allowed_auton = 1;
int distance_in_thebeginning = 0;
void controller_update() {
  while (true) {
    master.print(0,0, "%d          " , (int)distance_in_thebeginning);
    pros::delay(10);
  }

}
void matchloader(int time_out){
  pros::delay(500);
  int matchloader_distance = matchloader_color.get_proximity();
  bool condition = 40 < matchloader_distance && !(matchloader_color.get_hue() > 30 && matchloader_color.get_hue() < 150);
  int time_detected = pros::millis();
  int time_started = pros::millis();
  bool first_ball = false;
  while(pros::millis() - time_detected < 400 && pros::millis() - time_started < time_out){
    matchloader_color.set_led_pwm(100);
    matchloader_color.set_integration_time(3);
    matchloader_distance = matchloader_color.get_proximity();
    condition = 40 < matchloader_distance && !(matchloader_color.get_hue() > 30 && matchloader_color.get_hue() < 150);
    if (condition){first_ball = true;}

    if (first_ball){
      if (condition){time_detected = pros::millis();}
    } else {
      time_detected = pros::millis();
    }
    pros::delay(20);
  }
}
...
void matchloading_until_color(std::string color, int time_out) {
  ...
  bool condition;
  if (color == "B"){
    condition = 130 < matchloader_distance && matchloader_distance < 270;
  } else if (color == "R"){
    condition = matchloader_distance < 40 || matchloader_distance > 340;
  } 
  int time_detected = pros::millis();
  int time_started = pros::millis();
  bool first_ball = false;
  while(pros::millis() - time_detected < 5 && pros::millis() - time_started < time_out){
    matchloader_distance = matchloader_color.get_hue();
    if (color == "B"){
      condition = 130 < matchloader_distance && matchloader_distance < 270;
    } else if (color == "R"){
      condition = matchloader_distance < 40 || matchloader_distance > 340;
...
void scoring_timeout(int timeout){
  ...
  int time_started = pros::millis();
  bool no_ball = false;
  while(pros::millis() - time_started < timeout){
    condition = intake_top.get_current_draw() > 600 || intake_bottom.get_current_draw() > 600;
    if (!condition){
      if (!no_ball){
        time_detected = pros::millis();
      } 
      no_ball = true;
      if (no_ball && pros::millis() - time_detected > 300){
        break;
      }
    } else {
      no_ball = false;
...
void trapdoor_state(int trapdoor_val){
  trapdoor.set(trapdoor_val);
  scoring = trapdoor_val;
}

void intake_stop(){
  while (true) {
  while (trapdoor.get() == 0){
  if(intake_top_score.get_current_draw() > 2200){
    intake_score_allowed_auton = 0;
  }
  if (scoring == 0 && top_speed_score_intake < 0){
    intake_score_allowed_auton = 1;
  }
  if (intake_score_allowed_auton == 1){
    intake_top_score.move(top_speed_score_intake);
  }
  if (intake_score_allowed_auton == 0){
...
void default_constants() {
  ...
  chassis.pid_turn_exit_condition_set(100_ms, 1_deg, 350_ms, 3_deg, 500_ms, 500_ms);
...
void full_skills_auton(){
  ...
  chassis.pid_drive_set(57, 57);//62
  chassis.pid_wait();
  drive_wall_alternate(740, 100);//1080 
  drive_wall_alternate(800, 100);//1080 
  chassis.pid_drive_exit_condition_set(90_ms, 1_in, 200_ms, 3_in, 400_ms, 300_ms);
  
  chassis.pid_swing_set(RIGHT_SWING, 120, -80, 0);
  chassis.pid_wait();

  chassis.pid_drive_set(-9.5 , 100);
  chassis.pid_wait();

  pros::delay(100);

  chassis.pid_turn_set(-7,80);
  chassis.pid_wait();
  ...
  // drive_wall_back(1251, 80);
  // distance_in_thebeginning = distance_back_r.get_distance();
  chassis.pid_drive_set(39.5, 80);
  chassis.pid_wait();

  chassis.pid_turn_set(-44, 80);
  chassis.pid_wait_quick_chain();

  intake_piston.set(1);

  chassis.pid_drive_set(7.7, 100);//6.7
  chassis.pid_wait();
  ...
  chassis.pid_turn_set(-89, 100, true);
  chassis.pid_wait_quick_chain();
  
  chassis.pid_drive_set(28.5, 100, true);
  chassis.pid_wait_quick_chain();


  chassis.pid_swing_set(RIGHT_SWING, -130, 100, 20, true);
  pros::delay(200);
  ...
  chassis.pid_drive_set(30, 65, true);
  // trapdoor.set(0);
  // pros::delay(100);
  // trapdoor.set(1);
  matchloader(2000);
  ...
  chassis.pid_turn_set(185, 80, true);
  chassis.pid_wait_quick_chain();

  chassis.pid_drive_set(-42.5, 127, true);
  chassis.pid_wait_quick_chain();

  chassis.pid_turn_exit_condition_set(90_ms, 3_deg, 250_ms, 7_deg, 50_ms, 50_ms);
  chassis.pid_drive_exit_condition_set(90_ms, 1_in, 200_ms, 3_in, 40_ms, 30_ms);

  chassis.pid_swing_set(RIGHT_SWING, 0, -90, -10, true);
  chassis.pid_wait_quick_chain();
  ...
  chassis.pid_drive_set(-100, 127, true);
  pros::delay(200);
  ...
  chassis.pid_drive_exit_condition_set(90_ms, 1_in, 200_ms, 3_in, 40_ms,200_ms);
  chassis.pid_drive_set(50, 60);
  matchloader(2500);

  chassis.pid_drive_set(-27.5, 127);
  ...
  top_intake_score(127);
  Little_Mech_Mac.set(0);
  chassis.pid_drive_set(6.3, 127, true);
  chassis.pid_wait_quick_chain();
  top_intake_score(127);
  chassis.pid_swing_set(LEFT_SWING, 89 , 84, 31, true); //87, 84, 31
  chassis.pid_wait_quick();
  trapdoor.set(1);

  //clear the 6 ball from the park zone
  
  chassis.pid_drive_exit_condition_set(90_ms, 1_in, 200_ms, 3_in, 400_ms, 30000_ms);
  chassis.pid_drive_set(68, 57, true);
  intake_top_score.move(127);
  chassis.pid_wait();
  chassis.pid_drive_exit_condition_set(90_ms, 1_in, 200_ms, 3_in, 400_ms, 300_ms);
  ...
  // chassis.pid_wait();
  // // chassis.pid_turn_set(45, 100, true);
  // // chassis.pid_wait_quick_chain();
  ...
  chassis.pid_swing_set(LEFT_SWING, 20, 120, 10, true);
  chassis.pid_wait_quick_chain();
  chassis.pid_drive_set(-23, 80, true);
  chassis.pid_wait_quick_chain();
  chassis.pid_swing_set(LEFT_SWING, -15, -100, 30, true);
  chassis.pid_wait_quick_chain();
  chassis.pid_drive_set(-3, 100, true);
  chassis.pid_wait_quick_chain();
  chassis.pid_swing_set(LEFT_SWING, 45, 15, -110, true);
  chassis.pid_wait_quick_chain();

  // chassis.pid_swing_set(LEFT_SWING, 45, -15, -80, true);
  // chassis.pid_wait();
  // chassis.pid_swing_set(LEFT_SWING, 10, 117, 15, true);
  // chassis.pid_wait();
  // chassis.pid_swing_set(LEFT_SWING, 25, 117, 5, true);
  // chassis.pid_wait();
  ...
  intake_top.move_velocity(105);
  scoring_timeout(4000);
  pros::delay(600);
  bottom_intake(127);
  ...
  //New translation for the other side
  chassis.pid_drive_set(-8, 100, true);
  chassis.pid_wait_quick_chain();
  ...
  chassis.pid_turn_set(3, 80, true);
  chassis.pid_wait_quick_chain();
  ...
  chassis.pid_swing_set(RIGHT_SWING, 180, -90, -10, true);
  chassis.pid_wait_quick_chain();
  ...
  // chassis.pid_turn_set(4, 80, true);
  // chassis.pid_wait_quick_chain();
 ...
 chassis.pid_drive_set(7.3, 127, true);
  ...
  chassis.pid_swing_set(LEFT_SWING, -93, 84, 31, true); //-93, 84, 31
  chassis.pid_wait_quick_chain();
  ...
  intake_top_score.move(127);
  chassis.pid_drive_set(33, 85, true);
  chassis.pid_wait();
...
void left_4_3_push(){
   pros::Task color_sor(color_sort_top_auton);
  // pros::Task intake_stop1(intake_stop);
  discore_mech.set(1);
  trapdoor_state(1);
  ...
  chassis.pid_swing_set(RIGHT_SWING, -5, 127, 0, false);
  chassis.pid_wait_quick_chain();
  ...
  chassis.pid_swing_constants_set(9, 0.0, 90.5);
  chassis.pid_swing_set(LEFT_SWING, 180, 127, 21, true);
  chassis.pid_wait_quick_chain();
  ...
  while (pros::millis() - waittime  <= 1500) { 
    ...
    chassis.pid_drive_set(7.5, 80, true);
    chassis.pid_wait();
    ...
    chassis.pid_drive_set(-12, 50, false);
}
...
void left_7_mid(){
  ...
  chassis.pid_swing_set(LEFT_SWING, 175, 90, 30, true);
  chassis.pid_wait_quick_chain();
...
void left_elims_7ball(){
  ...
  chassis.pid_drive_set(-21, 80, true);
...
void left_9ball(){
  ...
  pros::delay(550);
  Little_Mech_Mac.set(1);
  matchloading_until_color(color, 500);
...
void right_4_3_push(){
  ...
  chassis.pid_drive_set(29, 100, true);
  chassis.pid_wait_quick_chain();
...
// old
void safe_skills(){
  pros::Task control(controller_update);
...
void intake_test(){
  ...
  //pros::delay(2600);
  int waittime = pros::millis();
  while (pros::millis() - waittime  <= 2600) { 
    if (color_sort.get_hue() > 210 && color_sort.get_hue() < 240){    
      break;
    }
  }
