...
void default_constants() {
  // P, I, D, and Start I
  chassis.pid_drive_constants_set(22, 0, 130);         // Fwd/rev constants, used for odom and non odom motions
  chassis.pid_heading_constants_set(11.0, 0.0, 20.0);        // Holds the robot straight while going forward without odom
  chassis.pid_turn_constants_set(4, 0.05, 30, 15.0);     // Turn in place constants
  chassis.pid_swing_constants_set(6.0, 0.0, 65.0);           // Swing constants
  chassis.pid_odom_angular_constants_set(2.4,0.0,34);    // Angular control for odom motions
  chassis.pid_odom_boomerang_constants_set(1.5, 0.0, 35);  // Angular control for boomerang motions
...
void auton_setup(){
  ...
  // chassis.pid_odom_set({{0_in, 10_in}, fwd, 30}, true);
  // chassis.pid_wait();

  // chassis.pid_odom_set({{16_in, 23_in}, fwd, 30}, true);
  // chassis.pid_wait();

  // chassis.pid_turn_set(162_deg, 30);
...
void blue_top_quals(){
  ...
  chassis.pid_odom_set({{{-16.5_in, 18_in}, rev, 127},
                      {{-11_in, 22_in}, rev, 127},
                      {{-13_in, 30_in}, rev, 110},
                      {{-12.5_in, 47_in}, rev, 110}},
                      true);
  chassis.pid_wait();
  ...
  chassis.pid_swing_set(RIGHT_SWING, 95, 60);
  chassis.pid_wait();

  intake_top.move(127);
  intake_bottom.move(127);

  chassis.pid_swing_set(LEFT_SWING, 160, 60, 8);
  chassis.pid_wait();

  chassis.pid_swing_set(RIGHT_SWING, 100, 60, 25);
  chassis.pid_wait();
  ...
  chassis.pid_odom_set({{13, 38}, fwd, 60}, true);
  chassis.pid_wait();
